...
You are the **relay** leaf under `main.core`: you build Lindenmayer's relay
client, `src/lindenmayer/core/relay.py`.

**Context, in reading order:**

- `docs/research/relay-integration/README.md` (the aggregate — especially
  recommendations 1 and 2) and `docs/DESIGN.md` §6. NEVER edit DESIGN.md.
- The frozen interface you build on (read the source):
  `lindenmayer.core.event.Event` (`from_dict`, `to_dict`, `verify`),
  `lindenmayer.core.keys.Keypair` (`sign_event`, `public_key_hex`),
  `lindenmayer.core.config.CoreConfig` (`has_capability`,
  `CAP_PRIVATE_READ_GATING`). Do NOT modify `event.py`, `keys.py`,
  `config.py`, or `core/__init__.py`, and do not create `kinds/` or
...
**Deliverable — `relay.py`, an asyncio client on `websockets`:**

- **Minimum relay contract: NIP-01 + NIP-29 + NIP-42 and NOTHING more.**
  Connect, auth, publish, subscribe, query:
  - NIP-01 wire protocol: `["EVENT",...]`, `["REQ", sub_id, filters...]`,
    `["CLOSE", sub_id]` client-side; `EVENT`/`EOSE`/`OK`/`NOTICE`/`CLOSED`
    relay-side. `publish` returns the OK result (raise or report rejection —
    fail loud, never swallow). `query` = REQ collecting until EOSE, then
    CLOSE. `subscribe` = async iterator of live events.
  - NIP-42: respond to `["AUTH", challenge]` with a signed kind-22242 event
    (`relay` + `challenge` tags) via the configured Keypair; support
    auth-required relays (retry the rejected op after auth completes).
  - NIP-29: group scoping via the `h` tag on filters and published events —
    a thin, explicit surface (e.g. a `group` parameter that adds the `h`
    tag), not a port of NIP-29 moderation. Join/leave/moderation events are
    OUT of the minimum contract.
- **Inbound events are verified before delivery** (`Event.verify()`);
  invalid events are dropped and counted, never surfaced as data.
- **Private-group read gating is verified, never inferred**: any API that
  treats a channel as private-readable (e.g. `require_private(group)` or a
  `private=True` publish/subscribe flag) MUST raise a loud, specific error
  unless `config.has_capability(CAP_PRIVATE_READ_GATING)` — the deployment
  operator's explicit attestation. No attestation, no private semantics —
  "NIP-29 compliant" alone is NOT enough. Non-private operation is
  unaffected.
- NIP-34, Blossom, and Buzz custom relay behaviors are additive capabilities
  layered elsewhere — do not implement or special-case them; keep the design
  extension-friendly (capability names live in config).
- Reconnect/backoff: keep it simple (bounded retries, resubscribe on
  reconnect); document what is NOT guaranteed (no persistent queue).

**Tests — `tests/test_relay.py` (+ helpers you own):**

- A mock relay: an in-process `websockets` server speaking enough NIP-01/42
  to exercise the client (accept EVENT->OK, REQ->stored EVENTs->EOSE, live
  fan-out, AUTH challenge flow, reject-until-authed).
- Cover: publish/OK round-trip; query until EOSE; live subscribe; NIP-42
  auth flow; invalid-signature event dropped; `h`-tag scoping applied;
  private-gating refusal without attestation and success with it.

**Constraints:**

- Files you own exclusively: `src/lindenmayer/core/relay.py`,
  `tests/test_relay*.py` (incl. any mock-relay helper module under tests/).
  Touch nothing else under src/ or tests/.
- No per-step publishing conveniences: the client publishes `Event`s it is
  given; do not add batching/telemetry helpers that would tempt per-step
  emission (privacy is a wire-format property — see DESIGN.md §6.1).
- Python 3.13 venv via uv — setup.sh handles it; `websockets` and
  `pytest-asyncio` are installed.
- Radio `main.core` for design questions (priority 5+); post real progress
  to your outbox each iteration.

## Completion Requirements

- `src/lindenmayer/core/relay.py` implements connect/auth/publish/subscribe/
  query per the minimum contract above, with fail-loud private-group gating.
- `bash $NODE_DIR/scripts/test.sh` passes, covering every bullet in the
  Tests section against the in-process mock relay.
